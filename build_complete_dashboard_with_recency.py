  ...
  <script>
    ...
    function renderOpenEndedData() {{
      ...
      windowDates.forEach((w, wIdx) => {{
        ...
        const top_25_pct = n === 0 ? 0 : Math.max(1, Math.round(n * 0.25));
      ...
      computedWindows.forEach((win, winIdx) => {{
        ...
        const hasFunds = win.n > 0 && topFunds.length > 0;
        const bestFund = hasFunds ? topFunds[0] : null;
        const bestName = bestFund ? `#1: ${{bestFund.schemeName}}` : "—";
        const bestCagr = (bestFund && bestFund.cagr !== null && bestFund.cagr !== undefined) ? `${{bestFund.cagr.toFixed(2)}}%` : "—";
        const hasChildren = hasFunds && topFunds.length > 1;

        const summaryTr = document.createElement("tr");
        summaryTr.className = `summary-row ${{hasChildren ? "cursor-pointer" : "cursor-default"}}`;
        if (hasChildren) {{
        ...
        summaryTr.innerHTML = `
          ...
          <td class="py-2.5 px-6 font-bold border-cell text-left text-slate-700">${{bestName}}</td>
    ...
    async function downloadExcelFile() {{
      ...
      activeComputedDataset.windows.forEach((w, winIdx) => {{
        ...
        const hasFunds = w.n > 0 && topFunds.length > 0;
        const bestFund = hasFunds ? topFunds[0] : null;
        const bestName = bestFund ? `#1: ${{bestFund.schemeName}}` : '—';
        ...
        if (bestFund && bestFund.cagr !== null && bestFund.cagr !== undefined) {{
          cTop.value = bestFund.cagr / 100.0;
          cTop.numFmt = '0.00%';
        }} else {{
          cTop.value = '—';
        }}
