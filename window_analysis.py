...
OUTPUT_EXCEL = "flexi_cap_3yr_rolling_analysis.xlsx"

START_YEAR, START_MONTH = 2015, 1
END_YEAR, END_MONTH = 2023, 9  # Windows up to 2023-09-01 -> 2026-09-01


def load_cache():
    if not os.path.exists(CACHE_FILE):
        raise FileNotFoundError(f"{CACHE_FILE} not found. Please run cache_history.py first.")
    with open(CACHE_FILE, "r", encoding="utf-8") as f:
        return json.load(f)


def parse_history(cache):
    parsed = {}
    for code, item in cache.items():
        ...
        for e in item.get("data", []):
            try:
                ...
                continue
        hist.sort(key=lambda x: x[0])  # Earliest date first
        parsed[code] = {
...
def get_nav_on_or_before(history, target_date, max_lookback_days=5):
    """Finds the most recent NAV on or before target_date (accounting for weekends/holidays)."""
...
def generate_windows():
    ...
    for yr in range(START_YEAR, END_YEAR + 1):
        for mo in range(1, 13):
            if yr == END_YEAR and mo > END_MONTH:
                break
...
def evaluate_window(parsed_data, w_start, w_end, quarter):
    eligible_results = []
    for code, fund in parsed_data.items():
        if not fund["earliest_dt"] or not fund["latest_dt"]:
            continue
        # Fund must have launched on or before start_date and survived until end_date
...
def create_excel_report(window_evaluations):
    ...
    # -------------------------------------------------------------------------
    # Sheet 1: 3Y Rolling Windows (Clean Layout without Window # column)
    # -------------------------------------------------------------------------
    ws1 = wb.active
    ws1.title = "3Y_Rolling_Windows"
    ws1.sheet_properties.outlinePr.summaryBelow = False  # Summary row at top, '+' expands downward

    # Title Banner
    ws1["A1"] = "Flexi Cap Mutual Funds — 3-Year Rolling Return Analysis (Regular Plans)"
    ws1["A1"].font = font_title
    ws1["A2"] = "Monthly Rolling Windows (2015-01-01 to 2026-09-01) | Click [+] on any row to expand Top 25% funds"
    ws1["A2"].font = font_subtitle

    # Headers on Row 4 (Window # removed, Quarter added, Minimal & Direct)
    headers = [
        "Start Date",
        "End Date",
        "Quarter",
    ...
    for win in window_evaluations:
        ...
        # Child Detail Rows: start from 2nd fund (top_funds[1:]) because #1 is on the summary row
        detail_start_row = current_row
        for rank, fund in enumerate(top_funds[1:], 2):
            detail_row = current_row
            
            # Blank out date and count columns for clean nested appearance
    ...
    # -------------------------------------------------------------------------
    # Bottom Section of ws1: Occurrences Grouped By Fund Name
    # -------------------------------------------------------------------------
    current_row += 3  # Add 3 empty rows as visual spacer

    ws1.cell(row=current_row, column=1, value="Flexi Cap Funds — Top 25% Consistency Summary (Grouped by Name)").font = font_title
    current_row += 1
    ws1.cell(row=current_row, column=1, value="Total occurrences in Top Quartile across all 105 monthly 3-year rolling windows (2015-01-01 to 2026-09-01)").font = font_subtitle
    current_row += 2
    ...
    # Compute occurrences and stats per fund name
    fund_stats = {}
    for win in window_evaluations:
    ...
    ranked_occurrences.sort(key=lambda x: (x["top_count"], x["consistency"]), reverse=True)

    for rank, item in enumerate(ranked_occurrences, 1):
        row_num = current_row
    ...
    print(f"\nReport successfully saved to {OUTPUT_EXCEL}")
...
def main():
    cache = load_cache()
    parsed_data = parse_history(cache)
    windows = generate_windows()

    print(f"Loaded {len(parsed_data)} schemes. Evaluating {len(windows)} monthly rolling windows...")

    evaluations = []
    for w_start, w_end, quarter in windows:
        res = evaluate_window(parsed_data, w_start, w_end, quarter)
        evaluations.append(res)

    create_excel_report(evaluations)
    print("Done!")
